...
static void loadImageGroup() {
	...
	gStoryScreenData.currentTextPos = getMugenDefVector2DOrDefaultAsGroup(gStoryScreenData.mCurrentGroup, "text.pos", Vector2D(22, 185));
...
static void loadTextGroup() {
	...
	if (gStoryScreenData.mNextVoice == 4)
	{
...
static void loadStoryScreen() {
	...
	streamMusicFile((std::string("game/TALK") + getStoryPlatformFilePath() + ".ogg").c_str());
